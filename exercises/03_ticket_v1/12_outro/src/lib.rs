...
impl Order {
    pub fn new (product_name: String, quantity: u32, unit_price: u32) -> Order {
        if product_name.is_empty() {
            panic!("Product name should not be is_empty!");
        }
        if product_name.capacity() > 300 {
            panic!("Product name should not be longer than 300 bytes");
        }
        if quantity <= 0 {
            panic!("Quantity should be greater than 0");
        }
        if unit_price <= 0 {
            panic!("Price should be in cents and greater than 0");
        }
    ...
    pub fn set_product_name(&mut self, new_product_name: String) {
        ...
        if new_product_name.capacity() > 300 {
            panic!("Product name should not be longer than 300 bytes");
        }
